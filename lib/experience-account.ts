...
const experienceDreams: ExperienceDreamSeed[] = [
  {
    title: "漂浮城市与会说话的鲸",
    contentPlain:
    ...
    shared: {
      anonymousProfileId: EXPERIENCE_ACCOUNT_PROFILE,
      title: "漂浮城市的最后一班列车",
...
async function upsertExperienceDream(userId: string, item: ExperienceDreamSeed) {
...
export async function ensureUniverseStory() {
...
async function resetExperienceUserRecord(userId: string) {
...
export async function seedExperienceAccount() {
  const passwordHash = await bcrypt.hash(EXPERIENCE_ACCOUNT_PASSWORD, 10);
  ...
  for (const item of experienceDreams) {
    await upsertExperienceDream(user.id, item);
  }

  await ensureUserDreamPreferences(user.id);
  await ensureUniverseStory();

  return user;
}

export async function resetExperienceAccountData(userId: string) {
  await prisma.$transaction([
    ...
    prisma.dream.deleteMany({ where: { userId } })
  ]);

  await prisma.universeStory.deleteMany({
    where: {
      sources: {
        none: {}
      }
  ...
  await resetExperienceUserRecord(userId);

  for (const item of experienceDreams) {
    await upsertExperienceDream(userId, item);
  }
